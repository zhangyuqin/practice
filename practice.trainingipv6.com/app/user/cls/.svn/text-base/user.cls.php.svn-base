...
class user_user
{
	...
	public function getUserList($page,$number = 20,$args = 1)
	{
		$page = $page > 0?$page:1;
		$r = array();
		$data = array(false,'user','openbasics',$args,'user.userid = openbasics.obuserid',false,'user.userid DESC',array(intval($page-1)*$number,$number));
		$sql = $this->sql->makeSelectLeft($data);
		$r['data'] = $this->db->fetchALL($sql,false,'userinfo');
		$data = array('COUNT(*) AS number','user','openbasics',$args,'user.userid = openbasics.obuserid',false,false,false);
		$sql = $this->sql->makeSelectLeft($data);
