...
function xvars(x){
	...
	var cellphone = function(x){
		return core(/^(13[0-9]|17[0-9]|15[0-9]|18[0-9])\d{8}$/,x);
	}
	...
	var fax = function(x){
		//传真
		return core(/^(\d{3,4}-)?\d{7,8}$/gi,x);
	}

	var code = function(x){
		//邮编
		return core(/^[1-9][0-9]{5}$/gi,x);
	}

	var landline = function(x){
		//座机
		return core(/^(\d{3,4}-)?(\d{7,8})(-\d{4})?$/gi,x);
	}
