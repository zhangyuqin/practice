...
class app
{
	...
	public function __construct(&$G)
	{
        ...
        //查询用户订单是否支付 作为判断课程是否开通的条件
// 		$userid = $this->_user['sessionuserid'];
// 		$orderargs = array(false,array('orders','openbasics'),array("orders.orderuserid = '{$userid}'","orders.orderbasicid = openbasics.obbasicid","orders.orderstatus = '2'"));
// 		$orderstatus = $this->order->getOrderByArgs($orderargs);
// 		$this->tpl->assign('orderstatus',$orderstatus);

		$this->tpl->assign('isopen',$this->data['openbasics']);
		...
		switch($action)
		{
		    ...
		    if($this->ev->get('payforit'))
		    {
		        ...
		        $message = array(
		            ...
		            "message" => "订单创建失败",
		            "callbackType" => 'forward',
		            "forwardUrl" => "index.php?user-center-payfor"
		        );
		        exit(json_encode($message));
		    }
			break;
...
// 					}
			...
			$this -> data['currentbasic']['basicexam']['opentime']['start'] = strtotime($this -> data['currentbasic']['basicexam']['opentime']['start']);
			$this -> data['currentbasic']['basicexam']['opentime']['end'] = strtotime($this -> data['currentbasic']['basicexam']['opentime']['end']);
			$this->tpl->assign('data',$this -> data);
			$this->tpl->assign('number',$number);
			$this->tpl->assign('exams',$exams);
