...
class favor_exam
{
	...
	//根据用户和科目获取考试记录列表（修改增加科目的查询）
	public function getExamHistoryListByArgs2($page,$number = 20,$args = array(),$fields = false)
	{
	    $page = $page > 0?$page:1;
	    $r = array();
	    $data = array($fields,array('examhistory','basic','subject'),$args,false,"ehid DESC",array(intval($page-1)*$number,$number));
	    $sql = $this->sql->makeSelect($data);
	    $r['data'] = $this->db->fetchAll($sql,'ehid');
	    $data = array('count(*) AS number',array('examhistory','basic','subject'),$args);
	    $sql = $this->sql->makeSelect($data);
	    $t = $this->db->fetch($sql);
	    $pages = $this->pg->outPage($this->pg->getPagesNumber($t['number'],$number),$page);
	    $r['pages'] = $pages;
	...
	//根据用户和科目获取考试记录列表（增加对用户表的查询）
	public function getAllExamHistoryUserByArgs($args = array(),$argsLeft,$fields = false)
	{
	    $data = array($fields,'examhistory','user',$args,$argsLeft,false,"ehscore DESC",false);
	    $sql = $this->sql->makeSelectLeft($data);
	    return $this->db->fetchAll($sql);
	}
