...
class app
{
	...
	public function exammanager()
	{
	    ...
	    switch($subaction)
	    {
            ...
            $this->tpl->assign('basics',$basics);
            //$this->tpl->assign('userid',$userid);
            $this->tpl->display('exams_manager');
            break;
            //开通考场
            case 'openbasics':
            $basicid = $this->ev->get('basicid');
            ...
            {
                $this->basic->modBasicOpen($basicid,1);
                $message = array(
                    'statusCode' => 200,
                    "message" => "开通成功",
                    "callbackType" => "forward",
                    "forwardUrl" => "index.php?exam-master-exammanager-basics"
                );
            }
            ...
            case 'closebasics':
            $basicid = $this->ev->get('basicid');
            $this->basic->modBasicOpen($basicid,0);
            $message = array(
                'statusCode' => 200,
                "message" => "考场已关闭",
                "callbackType" => "forward",
            ...
            if($basicid)
            {
                ...
                //$rs = $this->favor->getAllExamHistoryByArgs($args,array('userCandidateNumber','ehusername','ehscore','ehexam','ehstarttime'));
                $argsLeft =  "ehuserid = userid";
                $rs = $this->favor->getAllExamHistoryUserByArgs($args,$argsLeft,array('userCandidateNumber','ehusername','ehscore','ehexam','ehstarttime'));
                $r = array();
                foreach($rs as $p)
                {
                    $r[] = array('userCandidateNumber' => $p['userCandidateNumber'],'ehusername' => iconv("UTF-8","GBK",$p['ehusername']),'ehscore' => $p['ehscore'],'ehexam'=>iconv("UTF-8","GBK",$p['ehexam']),'ehstarttime' => date('Y-m-d',$p['ehstarttime']));
                }
	        ...
	        $page = $page > 1?$page:1;
	        $args = 1;
	        $subjects = $this->basic->getSubjectList($args);
	        if($search['basicid'])$args[] = "basicid = '{$search['basicid']}'";
	        else
	        {
	            if($search['keyword'])$args[] = "basic LIKE '%{$search['keyword']}%'";
	        ...
	        $page = $page > 0?$page:1;
	        //原来的$exams = $this->favor->getExamHistoryListByArgs($page,10,array("ehstatus = '1'","ehbasicid = '{$basicid}'"));
	        $args[] = "ehbasicid = {$basicid}";
	        $args[] = "basicid = ehbasicid";
	        $args[] = "subjectid = basicsubjectid";
	        $args[] = "ehtype = 2";
	        $exams = $this->favor->getExamHistoryListByArgs2($page,10,$args);
	        $this->tpl->assign('basicid',$basicid);
	        $this->tpl->assign('page',$page);
	        $this->tpl->assign('exams',$exams);
	        $this->tpl->display('exams_scorelist');
	        break;
	        case 'outscore':
            $args = array();
            $basicid = $this->ev->get('basicid');
            if($basicid)
            {
                ...
                //$rs = $this->favor->getAllExamHistoryByArgs($args,array('userCandidateNumber','ehusername','ehscore','ehexam','ehstarttime'));
                $argsLeft =  "ehuserid = userid";
                $rs = $this->favor->getAllExamHistoryUserByArgs($args,$argsLeft,array('userid','ehusername','ehscore','ehexam','ehstarttime'));
                $r = array();
                foreach($rs as $p)
                {
                    $r[] = array('userid' => $p['userid'],'ehusername' => iconv("UTF-8","GBK",$p['ehusername']),'ehscore' => $p['ehscore'],'ehexam'=>iconv("UTF-8","GBK",$p['ehexam']),'ehstarttime' => date('Y-m-d',$p['ehstarttime']));
                }
                if($this->files->outCsv($fname,$r))
                    $message = array(
                        'statusCode' => 200,
