...
class sql
{
	...
	//生成left join（新添加）
	public function makeSelectLeft($selectors,$type = 1)
	{
	    ...
	    if($selectors[2])$sql .= ' LEFT JOIN '.$this->tablepre.$selectors[2]." AS ".$selectors[2].' ON '.$selectors[4];
	
	    $sql .= " WHERE ";
	    if(!$selectors[3])$selectors[3] = 1;
	    if(is_array($selectors[3]))
	    {
	        $sql .= rtrim(implode(' AND ',$selectors[3]),'AND ');
	    }
	    else $sql .= $selectors[3];
	    ...
	    {
	        $sql .= " ORDER BY ";
	        if(is_array($selectors[6]))
	        {
	            $sql .= rtrim(implode(',',$selectors[6]),',');
	        }
	        else $sql .= $selectors[6];
	    }
	
	    if($selectors[7])
	    {
	        $sql .= " LIMIT ";
	        if(is_array($selectors[7]))
	        {
	            $sql .= rtrim(implode(',',$selectors[7]),',');
	        }
	        else $sql .= $selectors[6];
	    }
	    elseif($selectors[7] !== false)
	    {
	        $sql .= " LIMIT 0,100";
	    }
	    return $sql;
